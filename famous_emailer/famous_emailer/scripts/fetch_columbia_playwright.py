#!/usr/bin/env python3
"""
Playwright-based fetcher for Columbia Law profile pages.

Reads a CSV (default: data/columbia_names.csv) with columns:
  name,profile_url,email,source_url

For each profile_url it opens the page in Playwright, looks for an <a href="mailto:..."> and
writes results to:
  - data/columbia_contacts_playwright.csv
  - appends new contacts to data/law_contacts.csv (skips duplicate emails)

Usage:
  pip install playwright
  playwright install
  python scripts/fetch_columbia_playwright.py --mapping data/columbia_names.csv --headless
"""
...
def main(argv: List[str]) -> int:
    ...
    p.add_argument("--mapping", default="data/columbia_names.csv", help="CSV with profile_url column")
    p.add_argument("--output", default="data/columbia_contacts_playwright.csv")
    ...
    with sync_playwright() as p_handle:
        ...
        for idx, r in enumerate(rows, start=1):
            ...
            source_url = (r.get("source_url") or r.get("source") or profile_url).strip() or profile_url
            if not profile_url:
                continue
            ...
            for email in sorted(set(found_emails)):
                ...
                if lower_email not in existing_emails:
                    ...
                    law_rows_to_append.append({"name": name, "email": email, "profile_url": profile_url, "source_url": source_url, "source": "columbia_playwright"})
