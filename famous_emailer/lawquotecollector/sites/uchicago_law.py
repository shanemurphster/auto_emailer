...
PAGE_DELAY_S = 1.2
END_PAGE = 20  # Adjust if more pages
...
def _contacts_from_soup(soup: BeautifulSoup, page_url: str, affiliation: Optional[str]) -> List[Contact]:
	...
	# New approach for UChicago: names are present in div.profile-list--item__name.
	# For each such name element, find a surrounding profile container and look
	# for a mailto link inside that container. If no mailto is found, emit a
	# name-only contact so downstream flows can fill names into existing rows.
	name_elements = soup.select("div.profile-list--item__name")
	for ne in name_elements:
		name = (ne.get_text(" ", strip=True) or "").strip()
		if not name:
			continue
		# Ascend to a container that likely holds the email/link
		container = ne
		steps = 0
		email = None
		while isinstance(container, Tag) and steps < 6:
			# look for a mailto in this container
			email_link = container.select_one('a[href^="mailto:"]')
			if email_link:
				email = _extract_email(email_link.get("href", ""))
				break
		...
		if email and is_valid_email(email):
			...
			if key not in {c.email.strip().lower() for c in contacts if c.email}:
				contacts.append(Contact(name=name, email=email, affiliation=affiliation, source_url=page_url))
		else:
			# avoid duplicating name-only entries
			name_key = f"name::{name.strip().lower()}"
			if name_key not in { (c.name or "").strip().lower() for c in contacts }:
				contacts.append(Contact(name=name, email="", affiliation=affiliation, source_url=page_url))

	# Fallback: if we didn't get good matches above, try the original method
	if not contacts:
